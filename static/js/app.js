...
function getCkeditorForHost(host) {
    if (!host || typeof ClassicEditor === 'undefined' || !ClassicEditor.instances) return null;
    return ClassicEditor.instances.find(function (inst) {
        try { return host.contains(inst.ui.view.editable.element); } catch (e) { return false; }
    }) || null;
}

async function initAttachModalPaste(root, statementId, onContentChange) {
    if (!root) return;

    var modeToggle = root.querySelector('.paste-mode-toggle');
    var textBtn = root.querySelector('.paste-mode-btn[data-mode="text"]');
    var urlBtn = root.querySelector('.paste-mode-btn[data-mode="url"]');
    var textModeEl = root.querySelector('.paste-text-mode');
    var urlModeEl = root.querySelector('.paste-url-mode');
    var ckHost = root.querySelector('.rich-editor-host');
    var hiddenTextarea = root.querySelector('#att-content-hidden-' + statementId);
    var urlInput = root.querySelector('#att-content-url-' + statementId);
    var pasteIconBtn = root.querySelector('.paste-icon-btn');
    var pasteDropdownTrigger = root.querySelector('.paste-dropdown-trigger');
    var pasteDropdown = root.querySelector('.paste-dropdown-menu');
    var pastePlainItem = root.querySelector('[data-paste-action="plain"]');
    var formHint = root.querySelector('.paste-form-hint');
    var form = root.querySelector('#attach-form-' + statementId);
    ...
    if (textBtn) textBtn.addEventListener('click', function () { setPasteMode('text'); });
    if (urlBtn) urlBtn.addEventListener('click', function () { setPasteMode('url'); });

    if (pasteIconBtn) {
        pasteIconBtn.addEventListener('click', function () {
            pasteToAttachField(root, statementId, false);
        });
    }

    if (pasteDropdownTrigger) {
        pasteDropdownTrigger.addEventListener('click', function (e) {
            e.stopPropagation();
            var isOpen = pasteDropdown.classList.contains('show');
    ...
    if (pastePlainItem) {
        pastePlainItem.addEventListener('click', function () {
            pasteToAttachField(root, statementId, true);
            pasteDropdown.classList.remove('show');
            if (pasteDropdownTrigger) pasteDropdownTrigger.setAttribute('aria-expanded', 'false');
        });
    }
    ...
    if (form) {
        form.addEventListener('submit', function () {
            var activeModeBtn = root.querySelector('.paste-mode-btn.active');
            var mode = activeModeBtn ? activeModeBtn.dataset.mode : 'text';
            if (mode === 'url') {
                if (hiddenTextarea && urlInput) hiddenTextarea.value = urlInput.value;
            } else if (mode === 'text') {
                if (ckHost && window.COMPENDIUM_EDITORS) {
                    var editor = getCkeditorForHost(ckHost);
    ...
    if (ckHost && ckHost.dataset.ckeditorReady !== '1' && window.COMPENDIUM_EDITORS) {
        ckHost.dataset.ckeditorReady = '1';
        try {
            ckResult = await COMPENDIUM_EDITORS.renderRichText(ckHost, '', false);
            if (ckResult && ckResult.editor) {
                ckResult.editor.model.document.on('change:data', function () {
                    hiddenTextarea.value = ckResult.getData();
...
async function pasteToAttachField(root, statementId, asPlain) {
    if (!root) return;
    ...
    if (htmlFailed && formHint) {
        formHint.textContent = 'Formatting not available — pasted as plain text.';
        setTimeout(function () {
            if (formHint) formHint.textContent = 'Rich text note — formatting preserved.';
        }, 3000);
    }
